...
const getUserLocale = (): string => {
  ...
  // Extract language code (e.g., 'en-US' -> 'en', 'vi-VN' -> 'vi')
...
export const getTimeAgo = (date: Date | string): string => {
  ...
  return dayjs(date).locale(userLocale).fromNow();
};

export const formatDateDayjs = (date: Date | string): string => {
  const userLocale = getUserLocale();
  return dayjs(date).locale(userLocale).format('DD/MM/YYYY');
};
